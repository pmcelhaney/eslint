...
module.exports = {
    ...
    create(context) {
        ...
        const maxLength = options.code || 80,
            maxTokenCount = options.tokens || 0,
        ...
        function checkProgramForMaxTokenLength(node) {
            let currentLine = 0;
            let tokenCount = 0;
            const invalidLines = [];

            function reportLine(lineNumber, count) {
                context.report({
                    node,
                    loc: { line: lineNumber, column: 0 },
                    message: 'Line {{lineNumber}} has {{count}} non-puctuation tokens, which exeeds the maximum of {{maxTokenCount}}.',
                    data: {
                    lineNumber,
                    maxTokenCount,
                    count,
            ...
            sourceCode.ast.tokens.forEach((token) => {
                if (token.loc.start.line !== currentLine) {
                    ...
                    reportLine(currentLine, tokenCount);
                    }

                    currentLine = token.loc.start.line;
                    tokenCount = 0;
                }
        ...
        function checkProgramForMaxLength(node) {
            ...
            if (maxTokenCount > 0) {
                checkProgramForMaxTokenLength(node);
            }
